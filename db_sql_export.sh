...
do
  ...
  OP_STR="/usr/bin/pg_dump --file \"${DUMP_FOLDER}/${i}.sql\" --host ${DB_HOST} --port ${DB_PORT} --username ${DB_USERNAME} --data-only --disable-triggers --no-owner --no-privileges --no-tablespaces --no-unlogged-table-data --inserts --column-inserts --encoding \"UTF8\" --schema \"${DB_SCHEMA}\" \"${i}\""
